...
API_TOKEN=$(cat << RUBY_SCRIPT | podman --remote --url unix:///run/podman/podman.sock exec -i $CONTAINER_NAME bash -c 'cd /seek && RAILS_ENV=production bundle exec rails runner -'
