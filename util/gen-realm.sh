...
VARS='${PLATFORM_DOMAIN} ${PLATFORM_PROTOCOL} ${KC_SSL_REQUIRED} ${KEYCLOAK_CLIENT_SECRET} ${JUPYTERHUB_CLIENT_SECRET} ${AIRFLOW_KEYCLOAK_CLIENT_SECRET} ${SEEK_KEYCLOAK_CLIENT_SECRET} ${KEYCLOAK_REALM_ADMIN_PASSWORD} ${GRAFANA_OAUTH_SECRET} ${MINIO_KEYCLOAK_CLIENT_SECRET} ${ORTHANC_KEYCLOAK_CLIENT_SECRET}'
mkdir -p "$(dirname "$OUT")"
...
for v in PLATFORM_DOMAIN PLATFORM_PROTOCOL KC_SSL_REQUIRED KEYCLOAK_CLIENT_SECRET JUPYTERHUB_CLIENT_SECRET AIRFLOW_KEYCLOAK_CLIENT_SECRET SEEK_KEYCLOAK_CLIENT_SECRET KEYCLOAK_REALM_ADMIN_PASSWORD GRAFANA_OAUTH_SECRET MINIO_KEYCLOAK_CLIENT_SECRET ORTHANC_KEYCLOAK_CLIENT_SECRET; do
